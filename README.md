...
KiraAI 增强表情包插件。AI 通过 `<sticker_plus>` 标签按情绪发图（选图 LLM 从候选
描述中择优并直发），同时自动收藏聊天中他人发送的表情包，形成越用越懂你的表情包图库。
...
- **发送表情包**（`<sticker_plus>情绪</sticker_plus>` 标签）：AI 从 20 类固定情绪词表中
  给出情绪关键词，插件按情绪标签采样候选（6 张情绪匹配 + 随机补足），选图 LLM 基于候选的
  描述文本选出最贴切的一张并**直接发送**。发送与主模型异步：AI 的文字消息不必等待选图，
  表情包随后到达。发送结果（编号 + 描述）由插件以 `<system_reminder>` 记录追加到 AI 的
  消息之后进入历史上下文，主 AI 始终知道自己刚发了什么表情包。QQ 适配器以
  `image + sub_type=1`（收藏表情样式）发送，Telegram 以原生贴纸发送
- **偷表情**（`@on.im_message` hook）：他人发送的市场表情 / 表情包自动入库
  （普通图片不入库），WebUI 管理页可一键开关；可配置**入库需审核**——
  偷取的表情包先以禁用状态入库（仍会自动打标），在管理页人工启用后
  AI 才会使用，默认开启
...
| `steal_require_approval` | `true` | 偷取表情需审核：入库即禁用，人工启用后 AI 才可用（管理页也可开关） |
| `capacity` | `500` | 图库容量上限，满额淘汰低使用量表情 |
| `candidate_count` | `9` | 每次选图的候选数 |
| `max_emoji_size_mb` | `5.0` | 偷取单图大小上限（MB） |
| `vlm_model` | 空 | 打标模型（图片输入，需视觉能力），留空用系统默认 VLM |
| `selection_model` | 空 | 选图模型（从候选描述中挑选，纯文本任务无需视觉），留空用系统默认 LLM |
