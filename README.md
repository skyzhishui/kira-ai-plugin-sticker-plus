...
- **发送表情包**（`send_emoji` 工具 + `<sticker>` 标签）：AI 从 20 类固定情绪词表中
  给出情绪关键词，插件按情绪标签采样候选（6 张情绪匹配 + 随机补足），LLM 基于候选的
  描述文本选出最贴切的一张；AI 随后用 `<sticker>id</sticker>` 标签发出。QQ 适配器
  以 `image + sub_type=1`（收藏表情样式）发送，Telegram 以原生贴纸发送
